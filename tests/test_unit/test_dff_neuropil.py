...
from photon_mosaic_pipeline.rules.dff_run import calculate_dFF, dFF_gmm as dFF
...
def test_calculate_dFF_dset_separated_moving_window(tmp_path):
    """The dset_separated split also works with the moving_window method."""
    rng = np.random.default_rng(0)
    ...
    out_dset_dir = output_path.parent / "dset_separated"
    dff_0 = np.load(out_dset_dir / "dFF_dset0.npy")
    assert dff_0.shape == (2, 100)
